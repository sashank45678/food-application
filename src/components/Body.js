...
"https://www.swiggy.com/dapi/restaurants/list/v5?lat=16.766380388699826&lng=81.68431218713522&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING"
    ...
    const swiggydata =
      json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle
        ?.restaurants;
    const imagesdata=json?.data?.cards[0]?.card?.card?.imageGridCards.info
    setpageimages(imagesdata)
  ...
  return (
    <div>
      {pageimages && (<RestaurantPageImages images={pageimages}/>)}
    <div className="body">
      <div className="filter flex">
        ...
        <div className="flex items-center p-2">
          ...
          <input className="border border-black text-black ml-1 pl-1" onChange={(e)=>{
            setusername(e.target.value);
          }}></input>
        </div>
      </div>
      <div className="flex flex-wrap">
        {filterrestaurants.map((element, key) => (
          <div key={element.info.id}>
            ...
            >
              { element.info.avgRating>4?(<RestaurantcardPromoted element={element} />):(<Restaurantcard element={element} />)}
            </Link>
          </div>
        ))}
      </div>
